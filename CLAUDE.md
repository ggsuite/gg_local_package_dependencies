...
1. Enumerate the candidate directories (`packageCandidateDirs`): the immediate subdirectories of the input folder, plus the children of every immediate subdirectory that is a *grouping folder* — a visible directory that is neither a package nor a git repository. This makes the organization-folder layout `<workspace>/<org>/<repo>` transparent while a repository is never descended into, so packages nested inside a repo (`example/`, test fixtures, `node_modules`) stay invisible exactly as in a flat workspace. Fixtures: `test/sample_folder_orgs/`.
...
3. Load a `PackageManifest` per matching language (name + local deps + dev deps + the repository it declares); the first language stays primary. npm-scoped names are kept in full (`@scope/pkg`), so different scopes are distinct. When two folders claim one name, **both** paths are reported — kept and ignored — and the one that sits in the folder its own `repositoryUrl` names wins, whatever the path order is. That folder mismatch is the tell of a repository rename: the platform keeps redirecting the former name, so cloning it still succeeds and leaves a second checkout behind; the message says so only when the ignored folder actually names another repository, never for two unrelated packages that merely share a name.
